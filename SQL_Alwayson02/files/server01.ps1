...
#this is to change the drive letter of the CD so that we can use it for the disks
#$DVD_Drive = Get-WmiObject win32_volume -filter 'DriveLetter = "E:"'
#if you want to be more specific you can look for ' drivetype = 5' which is  the DVD
$DVD_Drive = Get-WmiObject win32_volume -filter 'DriveType = "5"'
$DVD_Drive.DriveLetter = "B:" # assign new letter you want
$DVD_Drive.Put()

Start-Sleep -Seconds 10        #we give a pause of 10 seconds so that the previous change is completed.
...
Install-WindowsFeature -Name Failover-Clustering –IncludeManagementTools –ComputerName server01
