...
class MyUser(HttpUser):
    ...
    def on_start(self):
        ...
        with self.client.post('/identity/v1/device/login', json=data, headers=headers,name='Identity',catch_response=True) as self.response:
            if self.response.status_code == 200:
                self.response.success()
                print(self.response)
            else:                
                self.response.failure('failed')           #this will log the error in the locust console
